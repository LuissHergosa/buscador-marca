...
const DocumentDetail: React.FC<DocumentDetailProps> = ({
  ...
  const { data: documentWithResults, isLoading: loadingResults } =
    useDocumentResults(document.id);
  ...
  const elementosPorRevisar = results
    .flatMap((result: BrandDetection) =>
    ...
    .sort((a, b) => a.pageNumber - b.pageNumber); // Sort by page number ascending

  const elementosRevisados = results
    .flatMap((result: BrandDetection) =>
      result.brands_detected
        .filter((brand: string) => result.brands_review_status[brand])
        .map((brand: string) => ({
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const blob = await documentApi.downloadExcel(document.id);
      ...
      const timestamp = new Date()
        ...
        .replace(/[:.-]/g, "")
        .slice(0, 15);
      const cleanName = document.filename
        .replace(".pdf", "")
        .replace(/[^a-zA-Z0-9]/g, "_");
      const filename = `${cleanName}_resultados_${timestamp}.xlsx`;
      ...
      alert("Error al descargar el archivo Excel. Inténtalo de nuevo.");
    }
  };
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 text-lg">
          Cargando análisis del documento...
        </p>
      </div>
    );
  }
              ...
              {elementosPorRevisar.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">
                    Todas las marcas han sido revisadas
                  </p>
                </div>
              ) : (
            ...
            <div className="space-y-4">
              {elementosRevisados.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h4 className="text-lg font-medium text-gray-900 mb-2">
                    Sin marcas revisadas
                  </h4>
                  <p className="text-gray-500">
                    Las marcas revisadas aparecerán aquí
                  </p>
                </div>
              ) : (
                <div className="grid gap-3">
